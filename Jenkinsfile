...
pipeline {
    ...
    environment {
      ...
      strDockerImage ="qkrdud2002/cicd_guestbook:${strDockerTag}"
    }
    stages {
        stage('Checkout') {
            steps {
                cleanWs()
                git branch: 'master', url: 'https://github.com/jihun81/guestbook.git'
            }
        }
        /*
        stage('Build') {
            steps {
                ...
                echo "jihun81/cicd_guestbook:${TODAY}_${BUILD_ID}"
            }
        }*/
        stage('Build_DOCKER') {
            agent { docker { 
          ...
          steps{
             withSonarQubeEnv('SonarQube-Server'){
               sh '''
                 ...
                 -Dsonar.login=4f8c96f263d04b6536f84877371e4a68970f42f3
                  '''
            }
          }
        }
       ...
       */
        ...
        stage('SSH Staging Server') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@192.168.56.140 whoami"
                    sh "ssh -o StrictHostKeyChecking=no root@192.168.56.140 env"
                }
            }
        }
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@192.168.56.140 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@192.168.56.140 docker container run \
                    -d \
                    -p 38080:80 \
                    --name=guestbookapp \
